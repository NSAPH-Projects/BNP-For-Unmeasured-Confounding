...
load("/n/dominici_nsaph_l3/Lab/projects/pm25-cardiovasculardisease-bnp/data_allariables_cleaned.RData")
...
data_clean<-data_clean[data_clean$Exposure>5 & data_clean$Exposure<14,]

# checking correlation among other variables
corrgram(data_clean[,c(4:28)], order=TRUE,
         lower.panel=panel.shade, upper.panel=panel.conf)
covariates<-c("PctFemale","smokerate2013","AvgCommute",
              "BMI2013","avgASOStemp","mean_age")
corrgram(data_clean[,covariates], order=TRUE,
         lower.panel=panel.shade, upper.panel=panel.conf)

# preparing dataset for the Gibbs

data_analysis=as.data.frame(cbind(Y=data_clean$lograte,
...
DDP_NOadj<-function(s_seed=1,X_tilde){
  ...
  #initialization
  alpha=matrix(rep(rmvnorm(1,alpha_mu*rep(1,dim_REG),
                           alpha_sigma*diag(dim_REG)),n_group),
  ...
  eta_X=cbind(eta_X,1)
  tau_prov=cbind(eta_X[,1]*dnorm(data_analysis$Y,X_tilde%*%(alpha[,1]),sqrt(gamma_y[1])),
                 eta_X[,2]*(1-eta_X[,1])*dnorm(data_analysis$Y,X_tilde%*%(alpha[,2]),sqrt(gamma_y[2])))
  ...
  for (r in 1:R){
    ...
    for (g_k in 1:n_group){
      if(length(which(K==g_k))==1){
        V=diag(dim_REG)/alpha_sigma+t(t(X_tilde[K==g_k,]))%*%t(X_tilde[K==g_k,])/gamma_y[g_k]
        M=alpha_mu/alpha_sigma+t(t(X_tilde[K==g_k,]))%*%t(data_analysis$Y[K==g_k])/gamma_y[g_k]
      }else{
        V=diag(dim_REG)/alpha_sigma+t(X_tilde[K==g_k,])%*%X_tilde[K==g_k,]/gamma_y[g_k]
        M=alpha_mu/alpha_sigma+t(X_tilde[K==g_k,])%*%data_analysis$Y[K==g_k]/gamma_y[g_k]
      }
      alpha[,g_k]=t(rmvnorm(1,solve(V)%*%M, solve(V)))
      
      G=sum((data_analysis$Y[K==g_k]-X_tilde[K==g_k,]%*%(alpha[,g_k]))^2)
...
DDP_ADJ<-function(s_seed=1, X_tilde,X_W_reg){
  ...
  for (r in 1:R){
    ...
    m_delta=delta_mu/delta_sigma+t(X_W_reg)%*%data_analysis$W/gamma_w
    ...
    if (dim_REG<4){
      intercept=sapply(1:n_group, function(g) 
        mean(alpha[1,g]+alpha[3,g]*mean(data_analysis$Z)+
...
# model Y|X,cov,W,Z
#X_tilde_YXCWZ=cbind(rep(1,n),data_analysis$X, data_analysis[,covariates],
#              data_analysis$W,data_analysis$Z)
...
#save.image("results_models_data.RData")
...
curve_NOadj<- function(x,post_chain,X_tilde){
  ...
  c_eta_X=pnorm(cbind(t(X_weights%*%(post_chain$post_eta[1:dim_Xw,])),
                      t(X_weights%*%(post_chain$post_eta[(dim_Xw+1):(dim_Xw*2),]))))
  for (g in 3:(n_group-1)) {
    c_eta_X=cbind(c_eta_X,t(pnorm(X_weights%*%(post_chain$post_eta[((g-1)*dim_Xw+1):(g*dim_Xw),]))))
  }
  ...
  if(dim_REG<3){
    ...
    values=sapply(1:length(clusters), function(c) 
      t(apply(X_tilde[,-2],2,mean))%*%intercept[((clusters[c]-1)*(dim_REG-1)+1):(clusters[c]*(dim_REG-1)),c]+
        x%*%beta[clusters[c],c])
  }
...
points_YX=sapply(points_x_i, function(x) 
  curve_NOadj(x,post_chain=model_YX, X_tilde=X_tilde_YX))
points_YXC=sapply(points_x_i, function(x) 
  curve_NOadj(x,post_chain=model_YXC, X_tilde=X_tilde_YXC))
points_YXU=sapply(points_x_i, function(x) 
  curve_NOadj(x,post_chain=model_YXU, X_tilde=X_tilde_YXU))
points_YXCU=sapply(points_x_i, function(x) 
  curve_NOadj(x,post_chain=model_YXCU, X_tilde=X_tilde_YXCU))
#points_YXCWZ=sapply(points_x_i, function(x) 
#  curve_NOadj(x,post_chain=model_YXCWZ, X_tilde=X_tilde_YXCWZ))
...
smooth_YXC=ksmooth(points_x_i,
                  apply(points_YXC,2, quantile, prob=0.5, na.rm=TRUE),
                  "normal", bandwidth = 0.5)$y
smooth_YXU=ksmooth(points_x_i,
                    apply(points_YXU[1:600,],2, quantile, prob=0.5, na.rm=TRUE),
                    "normal", bandwidth = 0.5)$y
smooth_YXCU=ksmooth(points_x_i,
                  apply(points_YXCU,2, quantile, prob=0.5, na.rm=TRUE),
                  "normal", bandwidth = 0.5)$y
#smooth_YXCWZ=ksmooth(points_x_i,
#                  apply(points_YXCWZ,2, quantile, prob=0.5, na.rm=TRUE),
#                  "normal", bandwidth = 0.5)$y

smooth_adj=ksmooth(points_x_i,
                  apply(points_adj,2, quantile, prob=0.5, na.rm=TRUE),
                  "normal", bandwidth = 0.5)$y
smooth_adjC=ksmooth(points_x_i,
                  apply(points_adjC,2, quantile, prob=0.5, na.rm=TRUE),
                  "normal", bandwidth = 0.5)$y
smooth_adjCW=ksmooth(points_x_i,
                    apply(points_adjCW,2, quantile, prob=0.5, na.rm=TRUE),
                    "normal", bandwidth = 0.5)$y
