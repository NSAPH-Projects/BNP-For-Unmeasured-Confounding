...
split_quantile <- function(x_matrix, probs = c(0.2, 0.4, 0.6, 0.8)){
  ...
  #        : split_quantile(x_matrix, probs = seq(0, 1, 0.25))
  #        : split_quantile(x_matrix, probs = c(0.15, 0.3, 0.45, 0.6, 0.75))
  seq_X = c(min(x_matrix), quantile(x_matrix, probs = probs), max(x_matrix))
  X_weights = cbind(
    x_matrix * I(x_matrix < seq_X[2]),
    x_matrix * I(x_matrix < seq_X[3] &
                   x_matrix > seq_X[2]),
...
split_quantile_x <- function(x_matrix, x, probs = c(0.2, 0.4, 0.6, 0.8)) {
  # example: split_quantile_x(x_matrix, x, probs = c(0.1, 0.25, 0.5, 0.75))
  #        : split_quantile_x(x_matrix, x, probs = seq(0, 1, 0.25))
