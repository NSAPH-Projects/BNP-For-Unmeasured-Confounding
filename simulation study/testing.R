    ...
    for (g_k in 1:n_group) {
      if (length(which(K == g_k)) == 1) {
        V = diag(3) / alpha_sigma + t(t(X_tilde[K == g_k, ])) %*% t(X_tilde[K ==
                                                                              g_k, ]) / gamma_y[g_k]
        M = alpha_mu / alpha_sigma + t(t(X_tilde[K == g_k, ])) %*% t(data_sim$Y[K ==
                                                                                  g_k]) / gamma_y[g_k]
      } else{
        V = diag(3) / alpha_sigma + t(X_tilde[K == g_k, ]) %*% X_tilde[K == g_k, ] /
          gamma_y[g_k]
        M = alpha_mu / alpha_sigma + t(X_tilde[K == g_k, ]) %*% data_sim$Y[K ==
                                                                             g_k] / gamma_y[g_k]
      }
      alpha[, g_k] = t(rmvnorm(1, solve(V) %*% M, solve(V)))
      
      G = sum((data_sim$Y[K == g_k] - X_tilde[K == g_k, ] %*% (alpha[, g_k])) ^
                2)
...
curve_chains <- function(X, data_sim, post_chian) {
  #X_design<-c(1, X-mean(data_sim$X))
  X_design <- c(1, X)
  eta_length <- length(X_design)
  c_eta_X = pnorm(cbind(t(X_design %*% (
    post_chian$post_eta[1:eta_length, (R_burnin + 1):R]
  )),
  t(X_design %*% (
    post_chian$post_eta[(eta_length + 1):(2 * eta_length), (R_burnin + 1):R]
  ))))
  for (g in 3:(n_group - 1)) {
    c_eta_X = cbind(c_eta_X, t(pnorm(X_design %*% (
      post_chian$post_eta[((g - 1) * eta_length + 1):(g * eta_length), (R_burnin +
                                                                          1):R]
    ))))
  }
  ...
  #values=sapply(1:length(clusters), function(c) post_chian$post_intercept[clusters[c],R_burnin+c]+
  #              X%*%post_chian$post_beta_x[clusters[c],R_burnin+c])
  values = sapply(1:length(clusters), function(c)
    t[c, ] %*% post_chian$post_intercept[, R_burnin + c] +
      t[c, ] %*% post_chian$post_beta_x[, R_burnin + c] * X)
